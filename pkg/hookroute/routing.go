...
const deferredToolHint = " If the anchored_* tools are not loaded yet (deferred — a direct call fails as not-found), FIRST run ToolSearch(query: \"+anchored_ execute fetch_and_index ctx_search\", max_results: 12) to load them, THEN make the call — do not stop using anchored or silently drop the task just because the schema was not loaded yet."
